...
cave=4.615561416657182e-06;
Cave=35.366366322981186;
Sin=5.667928139314932e+02;
...
%         Cdata=Nin+(Nend-Nin)*(Cdata-Nin)/(NendR-Nin);
        ...
        r = optimvar('r',8,"LowerBound",[1.0 0.025 15.0 1.0 5.0 0.2 3e-6 0.0003],"UpperBound",[2.5 0.05 35.0 10.0 10.0 0.25 0.001 0.001]);

        %r = optimvar('r',8,"LowerBound",[0.01 0.0 2.5 0.1 1.0 0.15 1e-6 1e-3],"UpperBound",[0.05 0.02 25.0 10.0 500.0 0.25 1e-4 0.1]);
        ...
        r0.r = [0.04 0.004 8.0 2.5 250.0 0.2 0.4e-4 0.0];
        ...
        ylim([-0.01 0.11]);
        legend('log(N)','log(N_f)','log(S)','D','T','E','Location','Eastoutside');
...
dlmwrite('varFLUCbMN.txt',reshape(solN,15,tEND),'\t');
dlmwrite('varFLUCbMS.txt',reshape(solS,15,tEND),'\t');
dlmwrite('varFLUCbMD.txt',reshape(solD,15,tEND),'\t');
dlmwrite('varFLUCbMT.txt',reshape(solT,15,tEND),'\t');
dlmwrite('varFLUCbME.txt',reshape(solE,15,tEND),'\t');

pars=[qest;fest;aest;pest;rest;dest;kest;best;qual]
dlmwrite('parsFLUCbM.txt',pars,'\t');
