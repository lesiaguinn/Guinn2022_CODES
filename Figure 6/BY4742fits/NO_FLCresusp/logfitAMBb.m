...
cave=3.555714431539862e-06;
Cave=1.343811832160505e+02;
Sin=5.666093811321886e+02;
...
%         end
        ...
        r = optimvar('r',7,"LowerBound",[0.001 0.01 0.2 0.1 1e-6 0.1 0.5],"UpperBound",[0.01 0.2 2.0 0.5 3.0 0.2 5]);
        ...
        r0.r = [0.004 0.05 0.4 0.75 1.5 0.2 1.0];
...
dlmwrite('varAMBbN.txt',reshape(solN,15,tEND),'\t');
dlmwrite('varAMBbS.txt',reshape(solS,15,tEND),'\t');
dlmwrite('varAMBbD.txt',reshape(solD,15,tEND),'\t');
dlmwrite('varAMBbT.txt',reshape(solT,15,tEND),'\t');
dlmwrite('varAMBbE.txt',reshape(solE,15,tEND),'\t');

pars=[qest;fest;aest;pest;rest;dest;kest;qual]
dlmwrite('parsAMBb.txt',pars,'\t');
