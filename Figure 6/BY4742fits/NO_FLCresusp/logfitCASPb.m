...
cave=2.923558478060808e-06;
Cave=1.710148540098206e+02;
Sin=5.938740047185896e+02;
...
for d=2:length(Stress)
    ...
    for n=1:3   % 3 replicates
        ...
        Nin=Cdata(1);
        Nend=Nin+Sin;
        %uncomment if you want to rescale growth curves
        ...
        r = optimvar('r',7,"LowerBound",[0.01 0.1 2.0 2.0 2.5 0.03 0.1],"UpperBound",[0.1 0.25 3.5 5.5 10.0 0.05 5.0]);
        ...
        r0.r = [0.004 8.0 4.0 0.75 1.5 0.04 1.0];
...
dlmwrite('varCASPbN.txt',reshape(solN,15,tEND),'\t');
dlmwrite('varCASPbS.txt',reshape(solS,15,tEND),'\t');
dlmwrite('varCASPbD.txt',reshape(solD,15,tEND),'\t');
dlmwrite('varCASPbT.txt',reshape(solT,15,tEND),'\t');
dlmwrite('varCASPbE.txt',reshape(solE,15,tEND),'\t');

pars=[qest;fest;aest;pest;rest;dest;kest;qual]
dlmwrite('parsCASPb.txt',pars,'\t');
