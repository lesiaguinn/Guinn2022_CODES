...
cave=5.882800938968053e-06;
Cave=-11.487904700015010;
Sin=506.6008230752486;
...
for d=2:length(Stress)
    ...
    for n=1:3   % 3 replicates
        ...
        r = optimvar('r',8,"LowerBound",[0.01 0.005 15.0 5.0 1.0 0.2 3e-6 4e-6],"UpperBound",[0.1 0.02 35.0 10.0 30.0 0.25 1e-5 8e-6]);
        ...
        r0.r = [0.05 0.004 25.0 4.0 5.0 0.1 0.05e-4 0.0];
...
dlmwrite('varFLUCbN.txt',reshape(solN,15,tEND),'\t');
dlmwrite('varFLUCbS.txt',reshape(solS,15,tEND),'\t');
dlmwrite('varFLUCbD.txt',reshape(solD,15,tEND),'\t');
dlmwrite('varFLUCbT.txt',reshape(solT,15,tEND),'\t');
dlmwrite('varFLUCbE.txt',reshape(solE,15,tEND),'\t');

pars=[qest;fest;aest;pest;rest;dest;kest;best;qual]
dlmwrite('parsFLUCb.txt',pars,'\t');
