...
cave=6.097037005146067e-06;
Cave=39.240741224361756;
Sin=5.321544760669708e+02;
...
for d=2:length(Stress)
    ...
    for n=1:3   % 3 replicates
        ...
        r = optimvar('r',7,"LowerBound",[0.0001 0.05 0.1 0.3 0.05 0.3 0.5],"UpperBound",[0.003 0.08 4.0 0.4 25.0 0.5 2.5]);
...
dlmwrite('varH2O2bMN.txt',reshape(solN,15,tEND),'\t');
dlmwrite('varH2O2bMS.txt',reshape(solS,15,tEND),'\t');
dlmwrite('varH2O2bMD.txt',reshape(solD,15,tEND),'\t');
dlmwrite('varH2O2bMT.txt',reshape(solT,15,tEND),'\t');
dlmwrite('varH2O2bME.txt',reshape(solE,15,tEND),'\t');

pars=[qest;fest;aest;pest;rest;dest;kest;qual]
dlmwrite('parsH2O2bM.txt',pars,'\t');
